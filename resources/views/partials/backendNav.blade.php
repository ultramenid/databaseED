<div class="border-b border-gray-300 sticky top-0 z-40 bg-white ">
    <div class="max-w-5xl mx-auto px-4"  x-data="{ pages: false }">
        <nav class=" flex space-x-6 text-sm leading-5 overflow-x-auto scrollbar-hide">
            <div class="whitespace-nowrap hover:bg-gray-200 flex flex-nowrap  hover:px-2 py-3 rounded w-full @if($nav == 'kasus' )border-b-2   border-newgray-900 @endif ">
                <a href="{{url('/cms/dashboard')}}" class=" px-0.5  @if($nav == 'kasus' )   text-black  @endif    cursor-pointer" >Database Kasus</a>
            </div>

            <div class="whitespace-nowrap hover:bg-gray-200 flex flex-nowrap  hover:px-2 py-3 rounded w-full @if($nav == 'ahli' )border-b-2   border-newgray-900 @endif ">
